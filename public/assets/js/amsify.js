...
(function(factory) {
    ...
    AmsifySuggestags.prototype = {
        ...
        _init: function() {
            if (this.checkMethod()) {
                this.name = $(this.selector).attr("name")
                    ? $(this.selector).attr("name") + "_amsify"
                    : "amsify_suggestags";
                this.createHTML();
                this.setEvents();
                $(this.selector).hide();
        ...
        createHTML: function() {
            var HTML =
                '<div class="' +
                this.classes.sTagsArea.substring(1) +
                '"></div>';
            this.selectors.sTagsArea = $(HTML).insertAfter(this.selector);
            var labelHTML =
                '<div class="' +
                this.classes.inputArea.substring(1) +
                '"></div>';
            ...
            var listArea =
                '<div class="' +
                this.classes.listArea.substring(1) +
                '"></div>';
            ...
            var list =
                '<ul class="' + this.classes.list.substring(1) + '"></ul>';
        ...
        setTagEvents: function() {
            ...
            $(this.selectors.sTagsInput).keyup(function(e) {
                ...
                var isDelimiter =
                    $.inArray(key, _self.settings.delimiters) !== -1
                        ? true
                        : false;
                if (key == "Enter" || key == "," || isDelimiter) {
                    ...
                    if (isDelimiter) {
                        $.each(_self.settings.delimiters, function(
                            dkey,
                            delimiter
                        ) {
                    ...
                    (_self.settings.suggestions.length ||
                        _self.isSuggestAction()) &&
                    ($(this).val() || _self.settings.showAllSuggestions)
                ) {
                    $(this).removeClass(
                        _self.classes.readyToRemove.substring(1)
                    );
        ...
        isSuggestAction: function() {
            return (
                this.settings.suggestionsAction &&
                this.settings.suggestionsAction.url
            );
        },
        ...
        getValue: function(tag) {
            if (this.settings.suggestions.length) {
                ...
                $.each(this.settings.suggestions, function(key, item) {
                    if (
                        typeof item === "object" &&
                        item.tag.toString().toLowerCase() == lower
                    ) {
                        value = item.value;
                        return false;
        ...
        processAjaxSuggestion: function(value, keycode) {
            ...
            var ajaxConfig = this.settings.suggestionsAction.callbacks
                ? this.settings.suggestionsAction.callbacks
                : {};
            var ajaxFormParams = {
                url: actionURL
            };
            ...
            if (this.settings.suggestionsAction.timeout !== -1) {
                ajaxFormParams["timeout"] =
                    this.settings.suggestionsAction.timeout * 1000;
            }
            if (
                this.settings.suggestionsAction.beforeSend !== undefined &&
                typeof this.settings.suggestionsAction.beforeSend == "function"
            ) {
                ajaxFormParams[
                    "beforeSend"
            ...
            ajaxFormParams["success"] = function(data) {
                if (data && data.suggestions) {
                    _self.settings.suggestions = $.merge(
                        _self.settings.suggestions,
                        data.suggestions
                    );
                    _self.settings.suggestions = _self.unique(
                        _self.settings.suggestions
                    );
                ...
                if (
                    _self.settings.suggestionsAction.success !== undefined &&
                    typeof _self.settings.suggestionsAction.success ==
                        "function"
                ) {
            ...
            if (
                this.settings.suggestionsAction.error !== undefined &&
                typeof this.settings.suggestionsAction.error == "function"
            ) {
                ajaxFormParams["error"] = this.settings.suggestionsAction.error;
            }
            ajaxFormParams["complete"] = function(data) {
                if (
                    _self.settings.suggestionsAction.complete !== undefined &&
                    typeof _self.settings.suggestionsAction.complete ==
                        "function"
                ) {
            ...
            if (keycode == "40" || keycode == "38") {
                ...
                this.delayTimer = setTimeout(function() {
                    if (_self.isSuggestAction() && !_self.ajaxActive) {
                        var minChars =
                            _self.settings.suggestionsAction.minChars;
                        var minChange =
                            _self.settings.suggestionsAction.minChange;
                        var lastSearch = _self.selectors.sTagsInput.attr(
                            "last-search"
                        );
                        if (
                            ...
                            (minChange === -1 ||
                                !lastSearch ||
                                _self.similarity(lastSearch, value) * 100 <=
                                    minChange)
                        ) {
                            _self.selectors.sTagsInput.attr(
                                "last-search",
                                value
                            );
        ...
        suggestWhiteList: function(value, keycode, showAll) {
            ...
            $(this.selectors.listArea)
                .find(_self.classes.noSuggestion)
                .hide();
            ...
            $list.find(this.classes.listItem).each(function() {
                ...
                if (
                    _self.settings.suggestMatch &&
                    typeof _self.settings.suggestMatch == "function"
                ) {
                    suggestItemResult = _self.settings.suggestMatch(
                        $(this).text(),
                        value
                    );
                } else {
                    suggestItemResult = ~$(this)
                        .text()
                        .toString()
                        .toLowerCase()
                        .indexOf(lower);
                ...
                if (
                    (all || suggestItemResult) &&
                    $.inArray(dataVal, _self.tagNames) === -1
                ) {
        ...
        createList: function() {
            ...
            if (_self.settings.noSuggestionMsg) {
                listHTML +=
                    '<li class="' +
                    _self.classes.noSuggestion.substring(1) +
                    '">' +
                    _self.settings.noSuggestionMsg +
                    "</li>";
            }
            return listHTML;
        },
        ...
        addTag: function(value, animate = true) {
            ...
            if (this.isPresent(value)) {
                ...
                this.setInputValue();
                if (
                    this.settings.afterAdd &&
                    typeof this.settings.afterAdd == "function"
                ) {
            ...
            $(this.selectors.listArea)
                ...
                .removeClass("active");
            $(this.selectors.listArea).hide();
            $(this.selectors.sTagsInput).removeClass(
                this.classes.readyToRemove.substring(1)
            );
            this.checkPlusAfter();
        },
        ...
        removeTag: function(value, animate = true) {
            var _self = this;
            $findTags = $(this.selectors.inputArea).find(
                '[data-val="' + value + '"]'
            );
            if ($findTags.length) {
        ...
        removeTagByItem: function(item, animate) {
            ...
            if (
                this.settings.afterRemove &&
                typeof this.settings.afterRemove == "function"
            ) {
        ...
        checkPlusAfter: function(fromBlur) {
            if (this.settings.showPlusAfter > 0) {
                if (this.tagNames.length > this.settings.showPlusAfter) {
                    var _self = this;
                    var plusNumber =
                        this.tagNames.length - this.settings.showPlusAfter;
                    if (!this.selectors.plusTag) {
                        var html =
                            '<span class="' +
                            this.classes.plusItem.substring(1) +
                            '"></span>';
                        this.selectors.plusTag = $(html).appendTo(
                            this.selectors.inputArea
                        );
                        $(this.selectors.plusTag).addClass(
                            this.classes.showPlusBg.substring(1)
                        );
                        /**
                    ...
                    $tagItems = $(this.selectors.inputArea).find(
                        this.classes.tagItem
                    );
                    $tagItems.show();
                    if (
                        $(this.selectors.inputArea).hasClass(
                            this.classes.focus.substring(1)
                        )
                    ) {
                        ...
                        if (fromBlur && !this.blurTgItems) {
                            ...
                            setTimeout(function() {
                                if (_self.blurTgItems) {
                                    _self.blurTgItems
                                        .slice(_self.settings.showPlusAfter)
                                        .hide();
                                }
                            }, 200);
                        } else {
